...
var Bar = React.createClass({
	...
	render: function() {
		var cells = this.props.data.streams.map(function(stream, i) {
			...
			var plot = stream && stream.state ? <div className={classes}></div> : null;
			var prevStream = this.props.prev.streams[i];
			var path = (prevStream && stream && prevStream.id === stream.id) ? <div className="path"></div> : null;
			...
			if (stream && stream.from) {
				...
				promisePath = <div className="promise-path" style={style}></div>;
			}
			...
			return (<div key={i} className="state">{plot}{path}{promisePath}</div>);
		}.bind(this));
		return (
			<div className="bar">{cells}
				<span className="message-streamid"> {'[' + this.props.data.streamId + '] '}</span>
				<span className="message-send">{this.props.data.send ? 'send' : 'recv'}</span>
				<span className="message-type"> {this.props.data.frameType} </span>
...
var App = React.createClass({
	...
	render: function() {
		...
		for (var i = 1; i < timeline.length; i++) {
			bars.push(<Bar key={i} prev={timeline[i - 1]} data={timeline[i]}/>);
		}
		return <div>{bars}</div>;
	}
});
